...
system.thermostat.set_langevin(kT=KT_reduced, gamma=Gamma_reduced, seed=44)
...
#Create lists and numpy arrays that will store the results of the simulation

temperature = []
energies = []
time = np.zeros(LOOPS)
rgs_results = np.zeros(LOOPS)
z=0
...
for i in range(LOOPS):
    ...
    energies.append(system.analysis.energy()['total'])      #Espresso function that computes the total energy of the system
    temperature.append((system.analysis.energy()['kinetic'])*(2./((3.*(id_count+num_q-3.)))))       #Espresso function that computes kinetic energy and is then used to calculate instantaneous temperature
    rgs_results[i] = system.analysis.calc_rg(chain_start=0,number_of_chains=1,chain_length=id_count)[0]     #Espresso function that computes the Radius of giration
...
#Turn off the thermostat and store temperature and energy in numpyu arrays

system.thermostat.turn_off()
ts = np.arange(0, LOOPS) * STEPS
